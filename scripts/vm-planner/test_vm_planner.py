...
def expected_vm_count(vp, cluster_type, topology_path=TOPOLOGY):
    _, cluster_types = vp.load_config(topology_path)
...
def run_internal_tests(vp):
    ...
    record(
        "vm_ip_from_offset within network",
        vp.vm_ip_from_offset("198.18.0.0/24", 30, 1) == "198.18.0.30",
        vp.vm_ip_from_offset("198.18.0.0/24", 30, 1),
    )
    try:
        vp.vm_ip_from_offset("198.18.0.0/24", 30, 227)
        record("vm_ip_from_offset rejects out-of-range", False, "no error raised")
    except ValueError as exc:
        record(
            "vm_ip_from_offset rejects out-of-range",
            "outside control-plane network" in str(exc)
            and "vm_number 227" in str(exc),
            str(exc),
        )

    append_sections = {
        "hv_vm": [
            "vm00001 ansible_host=hv02.example.com hv_ip=198.18.0.8 "
            "ip=198.18.0.30 vnc_port=5901 mac_address=52:54:00:00:00:01 "
            "domain_uuid=11111111-1111-1111-1111-111111111111",
            "vm00002 ansible_host=hv02.example.com hv_ip=198.18.0.8 "
            "ip=198.18.0.31 vnc_port=5902 mac_address=52:54:00:00:00:02 "
            "domain_uuid=22222222-2222-2222-2222-222222222222",
        ]
    }
    existing = vp.collect_existing_vm_identity(append_sections, 30, "198.18.0.0/16")
    record(
        "collect_existing_vm_identity tracks max vm id",
        existing["max_vm_number"] == 2 and existing["hv_vnc_max"]["hv02.example.com"] == 5902,
        str(existing["max_vm_number"]),
    )
    plan = [
        {
            "host": "hv02.example.com",
            "count": 1,
            "type": "master",
            "disk2_enable": True,
        }
    ...
    resolved = vp.resolve_append_generation_params(
        AppendArgs(),
        append_sections,
        "198.18.0.0/16",
        plan,
    ...
    record(
        "resolve_append_generation_params derives next vm id",
        resolved["start_vm_id"] == 3 and resolved["start_mac"] == "52:54:00:00:00:03",
        str(resolved),
    )
    try:
        AppendArgsBad = type(
        ...
        with patch.object(vp, "cli_flag_provided", lambda flag: flag == "--start-vm-id"):
            vp.resolve_append_generation_params(
                AppendArgsBad(),
                append_sections,
                "198.18.0.0/16",
                plan,
            )
        record("resolve_append rejects unsafe start-vm-id", False, "no error raised")
    except ValueError as exc:
        record(
            "resolve_append rejects unsafe start-vm-id",
            "would reuse existing VM identity" in str(exc),
            str(exc),
        )
    ...
    try:
        ...
        record("validate_vm_types rejects missing disk_location", False, "no error raised")
    except ValueError as exc:
        record(
            "validate_vm_types rejects missing disk_location",
            "must define 'disk_location'" in str(exc),
            str(exc),
        )
...
def run_start_at_hv_tests(vp):
    ...
    def validate_cluster_local_rotation(cluster_type, cluster_count, start_at_hv, label):
        ...
        try:
            ...
            hosts_by_cluster = plan_cluster_hosts(plan)
            ok = True
            details = []
    ...
    default_hosts = plan_cluster_hosts(plan_default)
    offset_hosts = plan_cluster_hosts(plan_offset)
    ...
    for index, line in enumerate(lines):
        if line.startswith("  cluster 1 (compact)"):
            block = [line]
    ...
    record(
        ...
        hvs[2]["hostname"].split(".")[0] in first_cluster_line,
        first_cluster_line,
    )
