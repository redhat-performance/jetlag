...
DEFAULT_TOPOLOGY_FILE = SCRIPT_DIR / "topology.json"

SECTION_RE = re.compile(r"^\[([^\]]+)\]$")
VAR_RE = re.compile(r"^([^=\s#]+)=(.+)$")


def topology_not_found_message():
    return (
        f"Topology file not found: {DEFAULT_TOPOLOGY_FILE}. "
        "Copy topology.json.sample to topology.json in the same directory as "
        "vm-planner.py, or pass --topology-file PATH."
    )


def resolve_topology_path(topology_file_arg):
    """Return topology file path from --topology-file or topology.json beside this script."""
    if topology_file_arg:
        return Path(topology_file_arg)
    if DEFAULT_TOPOLOGY_FILE.is_file():
        return DEFAULT_TOPOLOGY_FILE
    raise FileNotFoundError(topology_not_found_message())


def topology_source_label(topology_path):
    """Human-readable topology source for dry-run output."""
    return str(topology_path.resolve())
    ...
    for name, template in vm_types.items():
        ...
        for field in required_fields:
            if field not in template:
                raise ValueError(f"VM type '{name}' must define '{field}'")
...
# [hv_vm:vars] keys defined by Jetlag inventory templates (inventory-*.j2).
JETLAG_HV_VM_VARS_KEYS = frozenset(
    {
        "ansible_user",
        "ansible_ssh_pass",
        "base_domain",
...
# Per-host [hv_vm] keys consumed by hv-vm-create and manifest roles.
JETLAG_HV_VM_HOST_KEYS = frozenset(
    {
        "ansible_host",
        "hv_ip",
        "ip",
...
# vm-planner additions. vm_cluster_* avoids hv.yml playbook vars such as
# sno_cluster_count, compact_cluster_count, and standard_cluster_count.
VM_PLANNER_HV_VM_VARS_KEYS = frozenset({"vm_cluster_count", "vm_cluster_type_list", "vm_cluster_list"})
VM_PLANNER_HV_VM_HOST_KEYS = frozenset({"cluster"})
...
class InventoryOutput:
    ...
    def __init__(
        ...
        self.hv_vm_vars = hv_vm_vars or {}
        self.hv_vm_vars_key_order = None
...
def cluster_inventory_name(cluster_index):
    """Return the cluster= value for a VM inventory record."""
    return f"cluster{cluster_index}"


def deployment_cluster_metadata(deployment_plan):
    """Return vm_cluster_count, vm_cluster_type_list, and vm_cluster_list for [hv_vm:vars]."""
    if not deployment_plan:
        return {
            "vm_cluster_count": "0",
            "vm_cluster_type_list": "[]",
            "vm_cluster_list": "[]",
        }

    cluster_indices = sorted({entry["cluster_index"] for entry in deployment_plan})
    cluster_types = list(
        dict.fromkeys(entry["cluster_type"] for entry in deployment_plan)
    )
    cluster_names = [cluster_inventory_name(idx) for idx in cluster_indices]
    return {
        "vm_cluster_count": str(len(cluster_indices)),
        "vm_cluster_type_list": json.dumps(cluster_types),
        "vm_cluster_list": json.dumps(cluster_names),
    }


def parse_inventory_list(value):
    """Parse an Ansible inventory list value such as ["standard"]."""
    if not value:
        return []
    try:
...
def merge_hv_vm_vars(existing_vars, new_vars, append=False):
    """Merge generated [hv_vm:vars] with any existing inventory values."""
    merged = dict(existing_vars)
    if append:
        existing_count = int(existing_vars.get("vm_cluster_count", 0) or 0)
        new_count = int(new_vars.get("vm_cluster_count", 0) or 0)
        merged["vm_cluster_count"] = str(existing_count + new_count)
        merged["vm_cluster_type_list"] = json.dumps(
            list(
                dict.fromkeys(
                    parse_inventory_list(existing_vars.get("vm_cluster_type_list"))
                    + parse_inventory_list(new_vars.get("vm_cluster_type_list"))
                )
            )
        )
        merged["vm_cluster_list"] = json.dumps(
            list(
                dict.fromkeys(
                    parse_inventory_list(existing_vars.get("vm_cluster_list"))
                    + parse_inventory_list(new_vars.get("vm_cluster_list"))
                )
            )
        )
    else:
...
def format_hv_vm_vars_lines(vars_dict, existing_key_order=None):
    """Format the [hv_vm:vars] inventory section."""
    if not vars_dict:
        return []
    lines = ["[hv_vm:vars]"]
    ordered_keys = []
    for key in ("vm_cluster_count", "vm_cluster_type_list", "vm_cluster_list"):
        if key in vars_dict:
            ordered_keys.append(key)
    for key in existing_key_order or []:
        if key in vars_dict and key not in ordered_keys:
            ordered_keys.append(key)
...
def hv_vm_vars_key_order(sections):
    """Return [hv_vm:vars] key order from an inventory section map."""
    order = []
    for line in sections.get("hv_vm:vars", []):
...
def format_vm_inventory_blocks(inventory_output):
    ...
    if inventory_output.hv_vm_vars:
        if blocks:
            blocks.append("")
        blocks.extend(
            format_hv_vm_vars_lines(
                inventory_output.hv_vm_vars,
                inventory_output.hv_vm_vars_key_order,
            )
        )
    return blocks
...
def sno_cluster_number(entry):
    """Return the 1-based index used in sno-{n} VM names."""
...
def build_spread_deployment_plan(hvs, vm_types, cluster_types, selected_cluster_types, start_at_hv=0):
    ...
    for idx, instance in enumerate(instances):
        ...
        for vm_type, vm_count in instance["vms"].items():
            ...
            hv_cursor = (
                (hv_cursor + hypervisors_used_by_assignment(assignments)) % hv_count
                if hv_count
                else 0
            )
    return plan
...
def vm_ip_from_offset(network, offset, vm_number):
    ...
    host_offset = offset + vm_number - 1
    vm_ip = ipaddress.ip_address(int(network.network_address) + host_offset)
    if vm_ip not in network:
        raise ValueError(
            f"VM IP {vm_ip} is outside control-plane network {network} "
            f"(host offset {host_offset} = offset {offset} + vm_number {vm_number} - 1). "
            f"Reduce --vm-ip-offset or --start-vm-id, or use a larger network."
        )
    return str(vm_ip)
...
def build_vm_name(entry, type_num, vm_number, organize_vm_names, vm_prefix):
    ...
    if organize_vm_names:
        ...
        cluster_number = entry["cluster_index"]
...
def cli_flag_provided(flag):
    """Return True when a CLI flag appears in sys.argv."""
...
def parse_hv_vm_hosts(sections):
    """Return host records from the [hv_vm] inventory section."""
    hosts = []
    for line in sections.get("hv_vm", []):
        stripped = line.strip()
        if not stripped or stripped.startswith("#"):
            continue
        hostname, host_vars = parse_host_line(stripped)
        if hostname is None:
            continue
        hosts.append({"name": hostname, "vars": host_vars})
    return hosts


def empty_existing_vm_identity():
    """Return an empty collection of existing VM identity values."""
    return {
        "names": set(),
        "ips": set(),
        "macs": set(),
...
def collect_existing_vm_identity(sections, vm_ip_offset, network):
    """Collect identity values and per-hypervisor counters from [hv_vm] records."""
    existing = empty_existing_vm_identity()
    network_obj = ipaddress.ip_network(network, strict=False)
    network_base = int(network_obj.network_address)

    for record in parse_hv_vm_hosts(sections):
        vars_ = record["vars"]
        existing["names"].add(record["name"])
        if "ip" in vars_:
            existing["ips"].add(vars_["ip"])
            vm_num_from_ip = int(ipaddress.ip_address(vars_["ip"])) - network_base - vm_ip_offset + 1
            if vm_num_from_ip >= 1:
                existing["max_vm_number"] = max(existing["max_vm_number"], vm_num_from_ip)
        if "mac_address" in vars_:
            existing["macs"].add(vars_["mac_address"].lower())
        if "domain_uuid" in vars_:
            existing["uuids"].add(vars_["domain_uuid"].lower())

        hv_host = vars_.get("ansible_host")
        if hv_host:
            if "vnc_port" in vars_:
                vnc_port = int(vars_["vnc_port"])
                existing["hv_vnc_max"][hv_host] = max(
                    existing["hv_vnc_max"].get(hv_host, 0),
                    vnc_port,
                )
            existing["hv_vm_counts"][hv_host] = existing["hv_vm_counts"].get(hv_host, 0) + 1

        vm_match = re.match(r"^vm(\d+)$", record["name"], re.I)
        if vm_match:
            existing["max_vm_number"] = max(
                existing["max_vm_number"],
                int(vm_match.group(1)),
            )

    return existing


def derive_next_vm_id(existing, vm_prefix=None):
    """Return the next sequential VM ID from existing records, or None if unknown."""
    prefix = vm_prefix if vm_prefix is not None else "vm"
    max_from_names = 0
    name_pattern = re.compile(rf"^{re.escape(prefix)}(\d+)$", re.I)
    for name in existing["names"]:
        match = name_pattern.match(name)
        if match:
            max_from_names = max(max_from_names, int(match.group(1)))

    max_vm_number = max(existing["max_vm_number"], max_from_names)
...
def deployment_plan_hosts(deployment_plan):
    """Return hypervisor ansible_host values used by a deployment plan."""
    return {entry["host"] for entry in deployment_plan}


def resolve_append_generation_params(args, sections, network, deployment_plan):
    """Derive or validate append start values from existing [hv_vm] records."""
    existing = collect_existing_vm_identity(sections, args.vm_ip_offset, network)
    if not existing["names"]:
        return {
            "existing_identity": existing,
            "start_vm_id": args.start_vm_id,
            "start_mac": args.start_mac,
            "initial_hv_vnc_counters": None,
            "initial_hv_vm_counters": None,
        }

    derived_vm_id = derive_next_vm_id(existing, args.vm_prefix)
    if derived_vm_id is None:
        raise ValueError(
            "Cannot derive the next VM ID from existing [hv_vm] records. "
            "Set --start-vm-id explicitly to a value higher than any existing VM."
        )

    start_vm_id = derived_vm_id
    if cli_flag_provided("--start-vm-id"):
        if args.start_vm_id < derived_vm_id:
            raise ValueError(
                f"--start-vm-id {args.start_vm_id} would reuse existing VM identity "
                f"values (existing records use VM IDs through {derived_vm_id - 1}). "
                f"Use --start-vm-id >= {derived_vm_id}."
            )
        start_vm_id = args.start_vm_id

    if cli_flag_provided("--start-mac"):
        start_mac = args.start_mac
    elif existing["macs"]:
        max_mac_int = max(mac_to_int(mac) for mac in existing["macs"])
        start_mac = int_to_mac(max_mac_int + 1)
    else:
        start_mac = args.start_mac

    if cli_flag_provided("--start-mac"):
        new_vm_count = sum(int(entry["count"]) for entry in deployment_plan)
        for vm_number in range(start_vm_id, start_vm_id + new_vm_count):
            mac = vm_mac_address(vm_number, start_vm_id, start_mac).lower()
            if mac in existing["macs"]:
                raise ValueError(
                    f"--start-mac {start_mac} with --start-vm-id {start_vm_id} would "
                    f"reuse existing MAC address {mac}."
                )

    initial_hv_vnc_counters = {}
    initial_hv_vm_counters = dict(existing["hv_vm_counts"])
    for hv_host in deployment_plan_hosts(deployment_plan):
        if hv_host in existing["hv_vnc_max"]:
            required_vnc = existing["hv_vnc_max"][hv_host] + 1
            if cli_flag_provided("--start-vnc") and args.start_vnc < required_vnc:
                raise ValueError(
                    f"--start-vnc {args.start_vnc} would reuse VNC ports on {hv_host} "
                    f"(existing records use ports through {existing['hv_vnc_max'][hv_host]}). "
                    f"Use --start-vnc >= {required_vnc}."
                )
            initial_hv_vnc_counters[hv_host] = required_vnc
        elif cli_flag_provided("--start-vnc"):
            initial_hv_vnc_counters[hv_host] = args.start_vnc

    new_vm_count = sum(int(entry["count"]) for entry in deployment_plan)
    for vm_number in range(start_vm_id, start_vm_id + new_vm_count):
        vm_ip = vm_ip_from_offset(network, args.vm_ip_offset, vm_number)
        if vm_ip in existing["ips"]:
            raise ValueError(
                f"Planned VM IP {vm_ip} for VM ID {vm_number} is already used in "
                f"[hv_vm]. Adjust --start-vm-id or --vm-ip-offset."
            )
        vm_uuid = vm_domain_uuid(vm_number).lower()
        if vm_uuid in existing["uuids"]:
            raise ValueError(
                f"Planned domain_uuid for VM ID {vm_number} is already used in "
                f"[hv_vm]. Use a higher --start-vm-id."
            )

    return {
        "existing_identity": existing,
        "start_vm_id": start_vm_id,
        "start_mac": start_mac,
        "initial_hv_vnc_counters": initial_hv_vnc_counters or None,
        "initial_hv_vm_counters": initial_hv_vm_counters or None,
    }


def assert_new_vm_identity(name, vm_ip, vm_mac, vm_uuid, vnc_port, existing):
    """Reject generated VM values that collide with existing [hv_vm] records."""
    if not existing or not existing["names"]:
        return

    if name in existing["names"]:
        raise ValueError(f"VM name '{name}' already exists in [hv_vm]")
    if vm_ip in existing["ips"]:
        raise ValueError(f"VM IP {vm_ip} is already used in [hv_vm]")
    if vm_mac.lower() in existing["macs"]:
        raise ValueError(f"VM MAC {vm_mac} is already used in [hv_vm]")
    if vm_uuid.lower() in existing["uuids"]:
        raise ValueError(f"VM domain_uuid {vm_uuid} is already used in [hv_vm]")
...
def generate_inventory(
    ...
    hv_vnc_counters = dict(initial_hv_vnc_counters or {})
    hv_vm_counters = dict(initial_hv_vm_counters or {})
    ...
    for entry in deployment_plan:
        ...
        for type_num in range(1, entry["count"] + 1):
            ...
            cluster_suffix = f" cluster=cluster{cluster_index}"
            vm_line = (
    ...
    hv_vm_vars = deployment_cluster_metadata(deployment_plan)
    if sub_groups:
        return InventoryOutput(
            cluster_groups=cluster_groups,
            children=cluster_children,
            sub_groups=True,
            hv_vm_vars=hv_vm_vars,
        )
    return InventoryOutput(
        flat_lines=output_lines,
        sub_groups=False,
        hv_vm_vars=hv_vm_vars,
    )
...
def build_remote_resource_script(disk_paths):
    """Build a remote shell snippet reporting total CPU, memory, and disk capacity."""
    paths_shell = " ".join(shlex.quote(path) for path in disk_paths)
    return (
        "cpu=$(lscpu 2>/dev/null | awk '/^CPU\\(s\\):/ {print $2; exit}'); "
        '[ -z "$cpu" ] && cpu=$(awk "/^processor/ {c++} END {print c+0}" /proc/cpuinfo); '
        '[ -z "$cpu" ] || [ "$cpu" -eq 0 ] && cpu=$(nproc --all 2>/dev/null || nproc); '
        "mem_kb=$(awk '/MemTotal:/ {print $2; exit}' /proc/meminfo); "
        f"for path in {paths_shell}; do "
        'target="$path"; [ -e "$path" ] || target=$(dirname "$path"); '
        "total_kb=$(df -P \"$target\" 2>/dev/null | awk 'NR==2 {print $2}'); "
        '[ -n "$total_kb" ] && echo "DISK:$path:$((total_kb / 1024 / 1024))"; '
        "done; "
        'echo "CPU:$cpu"; '
...
def parse_remote_capacity(stdout):
    """Parse total CPU, memory, and per-path disk capacity from remote output."""
...
def fetch_hv_capacity(ip, user, password, disk_paths, timeout=30):
    """SSH to a hypervisor and return total CPU, memory, and disk capacity."""
    require_sshpass()
...
def assess_hv_warnings(load, capacity):
    ...
    if load["cpus"] > capacity["cpus"]:
        if capacity["cpus"] > 0:
            ...
            warnings.append(
                ...
                f"({load['cpus']} vCPUs planned, {capacity['cpus']} CPUs total)"
            )
        else:
            warnings.append(
    ...
    if memory_overcommitted(load, capacity):
        warnings.append(
            f"WARNING: memory exceeds capacity "
            f"({load['memory']} GB planned, {capacity['memory']} GB total)"
        )

    for path, planned in sorted(load.get("disk_by_path", {}).items()):
        total = capacity["disk_by_path"].get(path, 0)
        if planned > total:
            if total > 0:
                overcommit = planned / total
                warnings.append(
                    f"Disk overcommit on {path}: {overcommit:.2f}x "
                    f"({planned} GB planned, {total} GB total)"
                )
            else:
                warnings.append(
...
def memory_overcommitted(load, capacity):
    """Return True when planned VM memory exceeds total host memory."""
    return load["memory"] > capacity["memory"]


def print_memory_overcommit_abort(memory_overcommits, output_path):
    """Print a prominent warning when file creation is blocked."""
    border = "=" * 78
    print()
    print(border)
    print("WARNING: MEMORY OVERCOMMIT DETECTED — INVENTORY FILE NOT CREATED")
    print(border)
    print()
    print("The following hypervisors would exceed total physical memory:")
    print()
    for line in memory_overcommits:
        print(line)
    print()
    print(
        "Re-run with --dry-run to review the plan, reduce VM memory in your "
        "topology, or use --force to create the inventory file anyway "
        "(not recommended)."
    )
    print()
    print(f"Expected output file (not created): {output_path}")
    print(border)
    print()


def print_memory_overcommit_force_notice(memory_overcommits):
    """Print a notice when --force overrides a memory overcommit block."""
    border = "=" * 78
    print()
    print(border)
    print(
        "WARNING: --force was used; inventory file was created despite "
        "memory overcommit"
    )
    print(border)
    print()
    for line in memory_overcommits:
        print(line)
    print()
    print("Deployment with this inventory is not recommended.")
    print(border)
    print()
...
def print_hypervisors(
    ...
    When assess=True, also returns memory overcommit lines (host + warning).
    """
    ...
    for hv in hvs:
        ...
        for warning in assess_hv_warnings(load, capacity):
            print(f"    {warning}")
            if warning.startswith("WARNING: memory exceeds capacity"):
                memory_overcommits.append(f"  {short} ({ip}): {warning}")

    return memory_overcommits
...
def print_dry_run_summary(
    ...
    print(f"  Start VM ID    : vm{effective_start_vm_id:05d}")
    if args.organize_vm_names:
        ...
        print(f"  VM naming      : vm{{index:05d}} (from vm{effective_start_vm_id:05d})")
    if args.tag_roles:
...
def write_hv_vm_section(source_path, inventory_output, output_path, append=False):
    ...
    existing_hv_vm_vars = parse_section_vars(source_sections, "hv_vm:vars")
    inventory_output.hv_vm_vars = merge_hv_vm_vars(
        existing_hv_vm_vars,
        inventory_output.hv_vm_vars,
        append=append,
    )
    vars_key_order = hv_vm_vars_key_order(source_sections)
    inventory_output.hv_vm_vars_key_order = vars_key_order

    def format_vars_lines():
        return format_hv_vm_vars_lines(
            inventory_output.hv_vm_vars,
            vars_key_order,
        )
    ...
    in_hv_vm_vars_section = False
    vm_sections_written = False
    hv_vm_vars_written = False
    section_header_pending = False
    appended_in_section = False

    def append_generated_sections():
        nonlocal vm_sections_written, hv_vm_vars_written
        for block in format_vm_inventory_blocks(inventory_output):
            output.append(block + "\n")
        vm_sections_written = True
        hv_vm_vars_written = bool(inventory_output.hv_vm_vars)

    def append_hv_vm_vars_section():
        nonlocal hv_vm_vars_written
        if hv_vm_vars_written or not inventory_output.hv_vm_vars:
            return
        if output and not output[-1].endswith("\n"):
            output[-1] += "\n"
        for vars_line in format_vars_lines():
            output.append(vars_line + "\n")
        hv_vm_vars_written = True
    ...
    for line in lines:
        ...
        if section_match:
            ...
            if section_name == "hv_vm:vars":
                if append:
                    output.append(line if line.endswith("\n") else line + "\n")
                    in_hv_vm_vars_section = True
                    for vars_line in format_vars_lines()[1:]:
                        output.append(vars_line + "\n")
                    hv_vm_vars_written = True
                    continue
                if vm_sections_written:
                    in_hv_vm_vars_section = True
                    continue
                output.append(line if line.endswith("\n") else line + "\n")
                continue
...
def parse_args():
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Path to a topology file (JSON, YAML, or Python) with vm_types and "
            "cluster_types. Defaults to topology.json beside this script"
        ),
    )
    parser.add_argument(
        "--cluster-type",
        ...
        default="mno",
        help=(
            "Cluster type to deploy (default: mno). Must exist in cluster_types. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "clusters (e.g. cluster-1-master-1, sno-1)"
        ),
    )
    parser.add_argument(
        "--vm-prefix",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "[hv:vars] and compare planned load against total CPU, memory, and disk"
        ),
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help=(
            "With --assess-hv, create the inventory file even when planned VM "
            "memory exceeds total host memory (not recommended)"
        ),
    )
    parser.add_argument(
        "--start-at-hv",
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Host offset within controlplane_network for VM IPs (default: 30)",
    )
    parser.add_argument(
        "--start-vm-id",
        type=int,
...
def main():
    ...
    topology_path = resolve_topology_path(args.topology_file)
    vm_types, cluster_types = load_config(topology_path)
    ...
    if append_active:
        try:
            append_params = resolve_append_generation_params(
                args,
                parse_inventory(write_source),
                network,
                deployment_plan,
            )
        except ValueError as exc:
    ...
    inventory_output = generate_inventory(
        ...
        planned_vm_types,
        network=network,
        start_vm_id=append_params["start_vm_id"],
        vm_ip_offset=args.vm_ip_offset,
        start_vnc=args.start_vnc,
        start_mac=append_params["start_mac"],
        default_disk_vms=args.default_disk_vms,
        disk2_mount_path=args.disk2_mount_path,
        ...
        initial_hv_vnc_counters=append_params["initial_hv_vnc_counters"],
        initial_hv_vm_counters=append_params["initial_hv_vm_counters"],
        existing_identity=append_params["existing_identity"],
    )
    inventory_output.hv_vm_vars_key_order = hv_vm_vars_key_order(sections)
    ...
    if args.assess_hv:
        print()
        memory_overcommits = print_hypervisors(
            hvs,
            deployment_plan,
            vm_types,
            assess=True,
        ...
        if memory_overcommits and not args.force:
            print_memory_overcommit_abort(memory_overcommits, output_path)
            return 1
    ...
    if args.assess_hv and memory_overcommits and args.force:
        print_memory_overcommit_force_notice(memory_overcommits)
